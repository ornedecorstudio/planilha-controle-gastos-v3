...
export function categorizeMovimentacao(descricao, tipo, valor = 0) {
  ...
  // ===== PRÓ-LABORE (SAÍDA PJ → PF COM VALOR FIXO) =====
  // Valores exatos de pró-labore conforme contrato
  if (desc.includes('ERICK BESERRA') && tipo === 'saida' &&
      (Math.abs(valor - 20000) < 0.01 || Math.abs(valor - 7350) < 0.01)) {
    return {
      categoria: 'Pró-labore',
      subcategoria: 'Retirada Mensal',
      isReembolso: false,
      prioridade: 1
    };
  }
  ...
  if (desc.match(/JADLOG|TOTAL EXPRESS|AZUL CARGO|LOGGI|LOGGI TECNOLOGIA|MELHOR ENVIO|PRIME ENTREGAS/i)) {
    return {
      categoria: 'Fretes',
  ...
  // Lista conforme Contrato de Reembolso de Despesas Operacionais
  if (desc.match(/LARISSA.*OLIVEIRA|PAULA.*FALCAO|OLIVIA.*ALENCAR|ALEX SANDRO.*DANGUI|JOANNA.*NASCIMENTO|NATHALIA.*CAMPELO|MARILIA.*GONCALVES|MARIA ISABEL.*FERREIRA|RENNALLY.*ARAUJO/i) && tipo === 'saida') {
    return {
      categoria: 'Funcionários',
      subcategoria: 'Funcionário',
      isReembolso: false,
      prioridade: 2
    };
  }
