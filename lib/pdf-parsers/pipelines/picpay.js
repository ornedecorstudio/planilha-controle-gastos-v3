...
const TRANSACOES_EXCLUIR = [
  // Pagamentos NAO sao excluidos — sao transacoes reais classificadas como 'pagamento_fatura'
  // e participam da reconciliacao via formula: fatura_anterior - pagamento_fatura
...
/**
 * Tenta extrair uma transacao de um trecho de texto (remainder de uma linha).
 * Usado para recuperar transacoes da coluna direita quando a coluna esquerda
 * contem metadados (Subtotal, Total geral, etc).
 *
 * @param {string} remainder - Texto restante da linha apos o metadado
 * @param {number} lineNum - Numero da linha no texto original
 * @returns {object|null} Transacao extraida ou null
 */
function tentarExtrairTxnDeRemainder(remainder, lineNum) {
  if (!remainder) return null;
  const m = remainder.match(TX_PATTERN);
  if (!m) return null;

  let [, dataDDMM, desc, valorStr] = m;
  desc = desc.trim();

  if (FALSE_POSITIVE_DESCRIPTIONS.some(fp => desc.includes(fp))) return null;
  ...
  if (isNaN(valorOriginal) || valorOriginal === 0) return null;

  const valor = Math.abs(valorOriginal);

  const descUpper = desc.toUpperCase();
  if (TRANSACOES_EXCLUIR.some(e => descUpper.includes(e))) return null;

  return { line: lineNum, data: dataDDMM, descricao: desc, valor, valorOriginal };
}
...
function parsearSecoes(texto) {
  ...
  for (let i = 0; i < linhas.length; i++) {
    ...
    if (subMatch && current) {
      ...
      // CRITICAL: Em layout de 2 colunas, a coluna direita pode ter uma transacao
      // colada apos o subtotal da coluna esquerda. Ex:
      // "Subtotal dos lancamentos81,2503/12ALIEXPRESS-2.943,52"
      const afterSubtotal = trimmed.substring(subMatch.index + subMatch[0].length).trim();
      const recoveredTxn = tentarExtrairTxnDeRemainder(afterSubtotal, i);
      if (recoveredTxn) {
        current.txns.push(recoveredTxn);
        console.log(`[PicPay Pipeline] Recovered right-column txn from Subtotal line: ${recoveredTxn.data} ${recoveredTxn.descricao} ${recoveredTxn.valorOriginal}`);
      }
    ...
    if (current) {
      // Linhas a pular — mas verificar se ha transacao da coluna direita colada
      if (FULL_LINE_SKIP.some(s => trimmed.includes(s))) {
        for (const s of FULL_LINE_SKIP) {
          const idx = trimmed.indexOf(s);
          if (idx >= 0) {
            const afterSkip = trimmed.substring(idx + s.length).trim();
            const recoveredTxn = tentarExtrairTxnDeRemainder(afterSkip, i);
            if (recoveredTxn) {
              current.txns.push(recoveredTxn);
              console.log(`[PicPay Pipeline] Recovered right-column txn from skip line: ${recoveredTxn.data} ${recoveredTxn.descricao} ${recoveredTxn.valorOriginal}`);
            }
            break;
          }
        }
...
function deduplicarSecao(txns, subtotal) {
  ...
  const tolerance = subtotal * 0.001; // Tolerancia apertada (0.1%) — centavos de arredondamento
  ...
  for (const dk of dupeKeys) {
    ...
    for (let i = result.length - 1; i >= 0; i--) {
      ...
      // Nao remover se faria posSum cair abaixo do subtotal (previne undershoot)
      const afterRemoval = currentPosSum - result[i].valorOriginal;
      if (afterRemoval < subtotal - tolerance) {
        continue; // Pular esta duplicata — removê-la causaria undershoot
      }
 ...
 *
 * NOTA: IOF e checado ANTES de tarifa_cartao para que
 * "AJ A DEB TARIFA INTER IOF" seja classificado como iof, nao tarifa.
 */
function classificarTransacao(descricao, valorOriginal) {
  const d = descricao.toUpperCase();

  // Pagamento de fatura anterior (checado ANTES de tudo — sao transacoes reais negativas)
  if (/PAGAMENTO DE FATURA|PAGAMENTO RECEBIDO|PAGAMENTO EFETUADO/.test(d)) return 'pagamento_fatura';

  // IOF — checado ANTES de tarifa para que "AJ A DEB TARIFA INTER IOF" seja iof
...
function extrairFaturaAnterior(texto) {
  const regex = /Fatura\s+anterior\s*:?\s*R?\$?\s*(\d{1,3}(?:\.\d{3})*,\d{2})/i;
  const match = regex.exec(texto);
  if (match) return parseValorBR(match[1]);
  return null;
}

function extrairPagamentoRecebido(texto) {
  const regex = /Pagamento\s+recebido\s*:?\s*-?\s*R?\$?\s*(\d{1,3}(?:\.\d{3})*,\d{2})/i;
  const match = regex.exec(texto);
  if (match) return parseValorBR(match[1]);
  return null;
}
...
export function extractPipeline(texto, options = {}) {
  console.log('[PicPay Pipeline] v5 section-aware + value correction + fatura_anterior reconciliation');
  // === 1. Extrair metadados ===
  ...
  const faturaAnterior = extrairFaturaAnterior(texto);
  const pagamentoRecebido = extrairPagamentoRecebido(texto);

  console.log(`[PicPay Pipeline] Total fatura PDF: ${totalFaturaPDF}`);
  console.log(`[PicPay Pipeline] Despesas do mes: ${despesasDoMes}`);
  console.log(`[PicPay Pipeline] Fatura anterior: ${faturaAnterior}`);
  console.log(`[PicPay Pipeline] Pagamento recebido: ${pagamentoRecebido}`);
  console.log(`[PicPay Pipeline] Subtotais: ${JSON.stringify(subtotais)}`);

  // Resumo para reconciliacao: fatura_anterior mapeia para saldo_anterior em calcularAuditoria
  const resumoParaAuditoria = faturaAnterior ? {
    saldo_anterior: faturaAnterior,
    juros_anterior: 0,
    multas: 0,
  } : null;
  ...
  // === 4b. Classificar ANTES do global dedup (para proteger IOF/tarifas/estornos) ===
  todasTransacoes = todasTransacoes.map(t => {
    const tipo = classificarTransacao(t.descricao, t.valorOriginal);
  ...
  // === 4c. Global dedup usando soma_bruta como budget ===
  // A dedup por secao pode nao resolver tudo pois transacoes de uma secao
  // podem ter sido intercaladas em outra secao pelo layout de 2 colunas.
  //
  // Formula: total_fatura = fatura_anterior + soma_bruta(todas_txns)
  // Portanto: soma_bruta_esperada = total_fatura - fatura_anterior
  //
  // APENAS remove duplicatas classificadas como 'compra' — IOF, tarifas,
  // estornos e pagamentos sao preservados pois duplicatas nesses tipos
  // sao frequentemente legitimas (ex: IOF de compras internacionais).
  if (totalFaturaPDF && faturaAnterior) {
    const somaBrutaEsperada = totalFaturaPDF - faturaAnterior;
    const somaBrutaAtual = todasTransacoes.reduce((a, t) => {
      return a + (['estorno', 'pagamento_fatura'].includes(t.tipo_lancamento) ? -t.valor : t.valor);
    }, 0);
    const excesso = somaBrutaAtual - somaBrutaEsperada;
    const globalTolerance = 1.0;

    if (excesso > globalTolerance) {
      console.log(`[PicPay Pipeline] Global dedup: soma_bruta=${somaBrutaAtual.toFixed(2)}, esperada=${somaBrutaEsperada.toFixed(2)}, excesso=${excesso.toFixed(2)}`);

      // Contar duplicatas globais APENAS entre compras
      const keyCounts = {};
      for (const t of todasTransacoes) {
        if (t.tipo_lancamento !== 'compra') continue;
        const key = t.data + '|' + t.descricao + '|' + t.valor.toFixed(2);
        keyCounts[key] = (keyCounts[key] || 0) + 1;
      }
      ...
      let currentSomaBruta = somaBrutaAtual;
      for (const dk of dupeKeys) {
        if (currentSomaBruta <= somaBrutaEsperada + globalTolerance) break;

        for (let i = todasTransacoes.length - 1; i >= 0; i--) {
          if (currentSomaBruta <= somaBrutaEsperada + globalTolerance) break;
          if (todasTransacoes[i].tipo_lancamento !== 'compra') continue;

          const key = todasTransacoes[i].data + '|' + todasTransacoes[i].descricao + '|' + todasTransacoes[i].valor.toFixed(2);
          if (key !== dk.key) continue;

          const otherExists = todasTransacoes.some((t, j) => j !== i && t.tipo_lancamento === 'compra' &&
            (t.data + '|' + t.descricao + '|' + t.valor.toFixed(2)) === dk.key);
          if (!otherExists) continue;

          // Nao remover se faria soma_bruta cair abaixo do esperado
          const afterRemoval = currentSomaBruta - todasTransacoes[i].valor;
          if (afterRemoval < somaBrutaEsperada - globalTolerance) continue;

          console.log(`[PicPay Pipeline] Global dedup: removida compra ${dk.key}`);
          currentSomaBruta -= todasTransacoes[i].valor;
          todasTransacoes.splice(i, 1);
        }
      }

      console.log(`[PicPay Pipeline] Global dedup result: ${todasTransacoes.length} txns, soma_bruta=${currentSomaBruta.toFixed(2)} (target: ${somaBrutaEsperada.toFixed(2)})`);
    }
  }

  // === 5. Remover valorOriginal (nao necessario no output) ===
  todasTransacoes = todasTransacoes.map(({ valorOriginal, ...rest }) => rest);
  ...
  if (todasTransacoes.length >= 3) {
    ...
    const auditoria = calcularAuditoria(todasTransacoes, totalFaturaPDF, resumoParaAuditoria);
    auditoria.despesas_do_mes_pdf = despesasDoMes;
    auditoria.creditos_estornos_pdf = creditosEstornos;
    auditoria.fatura_anterior = faturaAnterior;

    // PicPay: tolerancia ampliada para layout de 2 colunas com centenas de transacoes
    // A dedup por secao + global nao e perfeita, mas chega muito perto (<0.05% do total)
    if (auditoria.diferenca_centavos !== null && Math.abs(auditoria.diferenca_centavos) <= 10000) {
  ...
  const auditoria = calcularAuditoria([], totalFaturaPDF, resumoParaAuditoria);
  auditoria.despesas_do_mes_pdf = despesasDoMes;
  auditoria.creditos_estornos_pdf = creditosEstornos;
  auditoria.fatura_anterior = faturaAnterior;
...
export function buildAIPrompt(cartaoNome, tipoCartao, metadados) {
  ...
  const faturaAnterior = metadados?.fatura_anterior
    ? `R$ ${metadados.fatura_anterior.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`
    : null;

  const pagamentoRecebido = metadados?.pagamento_recebido
    ? `R$ ${metadados.pagamento_recebido.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`
    : null;
  ...
  if (faturaAnterior) metadadosBloco += `\n- Fatura anterior: ${faturaAnterior}`;
  if (pagamentoRecebido) metadadosBloco += `\n- Pagamento recebido: ${pagamentoRecebido}`;
...
- "pagamento_fatura": pagamento de fatura anterior (ex: "PAGAMENTO DE FATURA PELO PICPAY")
...
NOTA: NAO exclua "PAGAMENTO DE FATURA PELO PICPAY" — classifique como "pagamento_fatura".
</items_to_exclude>
...
export function parsePicPay(texto) {
  ...
  return {
    ...
    resumo_fatura: {
      ...
      pagamento_fatura: resultado.auditoria?.pagamento_fatura || 0,
      saldo_anterior: resultado.auditoria?.saldo_anterior || 0,
      ...
      fatura_anterior: resultado.metadados_verificacao.fatura_anterior,
      reconciliado: resultado.auditoria?.reconciliado || false,
      diferenca_centavos: resultado.auditoria?.diferenca_centavos || null,
      equacao: resultado.auditoria?.equacao || 'Parser PicPay pipeline v5',
