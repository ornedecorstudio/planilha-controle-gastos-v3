...
function extrairTotalFaturaPDF(texto) {
  ...
  // Fallback 3: "Sua fatura é de R$ X.XXX,XX" ou "fatura é R$ X.XXX,XX"
  const regexFaturaE = /(?:sua\s+fatura\s+(?:é|e)\s+(?:de\s+)?|fatura\s+(?:é|e)\s+(?:de\s+)?)R\$\s*(\d{1,3}(?:\.\d{3})*,\d{2})/gi;
  while ((match = regexFaturaE.exec(textoNorm)) !== null) {
    ultimoValor = parseValorBR(match[1]);
  }
  if (ultimoValor !== null) {
    console.log(`[c6bank] Total extraido via fallback "Sua fatura é de": ${ultimoValor}`);
    return ultimoValor;
  }

  // Fallback 4: "R$ X.XXX,XX" logo após "Total" (C6 pode juntar sem espaço)
  const regexTotalColado = /Total\s*R\$\s*(\d{1,3}(?:\.\d{3})*,\d{2})/gi;
  while ((match = regexTotalColado.exec(textoNorm)) !== null) {
    ultimoValor = parseValorBR(match[1]);
  }
  if (ultimoValor !== null) {
    console.log(`[c6bank] Total extraido via fallback "Total R$": ${ultimoValor}`);
    return ultimoValor;
  }
