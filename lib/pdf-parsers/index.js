...
export function detectarBanco(texto) {
  ...
  // C6 Bank — ANTES de PicPay para evitar falso positivo
  // (faturas C6 podem conter "PICPAY*" como nome de merchant em transações,
  //  ex: "PICPAY*ORNE DECOR ST 313,47" que disparava detecção PicPay)
  ...
  // PicPay — ANTES de Renner e Santander para evitar falso positivo
  // (faturas PicPay contêm transações em "LOJAS RENNER", "SANTANDER" etc
  //  como nomes de merchant, que disparavam detecção errada)
  // "PICPAY SERVICOS", "PIC PAY", "PICPAY BANK", "PICPAY CARD" são específicos de faturas PicPay.
  // "PICPAY" genérico é mantido mas só chega aqui se C6 não matchou antes.
  if (textoUpper.includes('PICPAY SERVICOS') ||
      textoUpper.includes('PICPAY BANK') ||
      textoUpper.includes('PICPAY CARD') ||
      textoUpper.includes('PIC PAY') ||
      textoUpper.includes('PICPAY')) {
    return 'picpay';
  }

  // Renner / Realize CFI — depois de PicPay para evitar falso positivo
  // (faturas PicPay podem conter "LOJAS RENNER" como nome de merchant)
  // NOTA: removida keyword "LOJAS RENNER" que é nome de loja/merchant genérico
  // e aparece em faturas de outros bancos como transação de compra.
  // Keywords restantes são específicas do emissor Realize/Renner.
